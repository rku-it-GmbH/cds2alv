CLASS /c2a/cl_grid_builder DEFINITION
  PUBLIC
  INHERITING FROM /c2a/cl_base_object
  CREATE PUBLIC.

  PUBLIC SECTION.
    INTERFACES /c2a/if_grid_builder.
    ...
    METHODS constructor
      IMPORTING i_cds_view         TYPE /c2a/cds_view_name
                ...
                i_value_help       TYPE REF TO /c2a/if_value_help
                i_action_handler   TYPE REF TO /c2a/if_action_handler
                i_navigation       TYPE REF TO /c2a/if_navigation
      RAISING   /c2a/cx_error_message.

  PROTECTED SECTION.
    TYPES: BEGIN OF ty_field_properties,
             fieldname           TYPE fieldname,
             fieldtype           TYPE /c2a/field_type,
    ...
    DATA event_handlers         TYPE /c2a/grid_event_handler_tab.
...
CLASS /c2a/cl_grid_builder IMPLEMENTATION.
  METHOD build_event_handler.
    DATA(report_info) = persistence->get_report_for_cds_view( cds_view ).
    DATA(functions_display_mode) = report_info-add_functions_display_mode.
    DATA(event_handler) = NEW /c2a/cl_alv_grid_event_handler( i_cds_view               = cds_view
                                                              i_alv_grid               = alv_grid
  ...
  METHOD evaluate_annotations.
    ...
    LOOP AT element_annotations ASSIGNING FIELD-SYMBOL(<element_annotation_group>)
      ...
      DATA(semantic_object) = VALUE /c2a/semantic_object( ).
      DATA(function) = VALUE /c2a/function( ).
      ...
      LOOP AT ui_annotations INTO DATA(ui_index) GROUP BY ui_index-index.
        DATA(field_action) = VALUE /c2a/field_action( cds_view  = cds_view
                                                      fieldname = fieldname ).
    ...
    IF update_enabled = abap_true.
      LOOP AT ddfields ASSIGNING FIELD-SYMBOL(<ddfield>).
        ...
        ASSIGN field_properties_table[ fieldname = <ddfield>-fieldname ] TO FIELD-SYMBOL(<field_properties>).
        IF sy-subrc = 0.
  ...
  METHOD /c2a/if_grid_builder~create_alv_grid.
  ...
  METHOD /c2a/if_grid_builder~get_gui_title.
    r_title = description.
  ENDMETHOD.

  METHOD /c2a/if_grid_builder~get_metadata.
    table_container = i_table_container.
  ...
  METHOD /c2a/if_grid_builder~register_alternative_selection.
