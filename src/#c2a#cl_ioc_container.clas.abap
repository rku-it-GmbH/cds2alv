CLASS /c2a/cl_ioc_container DEFINITION
  PUBLIC
  CREATE PUBLIC.

  PUBLIC SECTION.
    INTERFACES /c2a/if_ioc_container.

    ALIASES resolve    FOR /c2a/if_ioc_container~resolve.
    ALIASES ty_filters FOR /c2a/if_ioc_container~ty_filters.

    DATA cds_view TYPE /c2a/cds_view_name READ-ONLY.

    METHODS constructor
      IMPORTING i_cds_view TYPE /c2a/cds_view_name           OPTIONAL
                i_parent   TYPE REF TO /c2a/if_ioc_container OPTIONAL.
...
CLASS /c2a/cl_ioc_container IMPLEMENTATION.
  METHOD check_class.
    TRY.
      ...
      CATCH cx_class_not_existent.
        RAISE EXCEPTION TYPE /c2a/cx_error_message MESSAGE e027(/c2a/cds_alv) WITH i_class.
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD check_implements.
    ...
    IF     NOT line_exists( if_relations->implementing_classes[ clsname = i_class ] )
      ...
      RAISE EXCEPTION TYPE /c2a/cx_error_message MESSAGE e029(/c2a/cds_alv) WITH i_class i_interface.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD check_interface.
    TRY.
      ...
      CATCH cx_class_not_existent.
        RAISE EXCEPTION TYPE /c2a/cx_error_message MESSAGE e028(/c2a/cds_alv) WITH i_interface.
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD create_object.
    ...
    TRY.
        DATA(implementation) = implementations[ interface = i_interface
                                                filters   = i_filters ].
        ...
        LOOP AT get_constructor_parameters( implementation-class ) INTO DATA(parameter).
          ...
          IF is_interface( parameter-type ).
            ...
            INSERT VALUE #( name  = parameter-sconame
                            value = ref_to_object )
                   INTO TABLE parameters.
          ENDIF.
        ENDLOOP.
      ...
      CATCH cx_sy_itab_line_not_found cx_class_not_existent.
        RAISE EXCEPTION TYPE /c2a/cx_error_message MESSAGE e016(/c2a/cds_alv) WITH i_interface.
      CATCH cx_sy_create_object_error.
        RAISE EXCEPTION TYPE /c2a/cx_error_message MESSAGE e017(/c2a/cds_alv) WITH i_interface implementation-class.
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD get_return_type.
    ...
    TRY.
        IF interface_name IS NOT INITIAL AND method_name IS NOT INITIAL.
          ...
          r_return_type = interface->method_parameters[ cmpname    = method_name
                                                        pardecltyp = seos_pardecltyp_returning ]-type.
        ELSEIF class_name IS NOT INITIAL AND method_name IS NOT INITIAL.
          DATA(class) = cl_oo_class=>get_instance( CONV #( class_name ) ).
          r_return_type = class->method_parameters[ cmpname    = method_name
                                                    pardecltyp = seos_pardecltyp_returning ]-type.
        ENDIF.

        check_interface( r_return_type ).
      CATCH cx_class_not_existent cx_sy_itab_line_not_found.
        RAISE EXCEPTION TYPE /c2a/cx_error_message MESSAGE e030(/c2a/cds_alv) WITH method_name.
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD store_object.
    ...
    IF NOT interface_descriptor->applies_to( i_object ).
      RAISE EXCEPTION TYPE /c2a/cx_error_message MESSAGE e015(/c2a/cds_alv) WITH i_interface.
    ENDIF.

    ASSIGN instances[ interface = i_interface
                      filters   = i_filters ] TO FIELD-SYMBOL(<instance>).
    IF sy-subrc <> 0.
      INSERT VALUE #( interface = i_interface
                      filters   = i_filters )
  ...
  METHOD /c2a/if_ioc_container~register_implementing_class.
    check_interface( i_interface ).
    check_class( i_class ).
    check_implements( i_class     = i_class
                      i_interface = i_interface ).

    ASSIGN implementations[ interface = i_interface
                            filters   = i_filters ] TO FIELD-SYMBOL(<implementation>).
    IF sy-subrc <> 0.
      INSERT VALUE #( interface = i_interface
                      filters   = i_filters )
  ...
  METHOD /c2a/if_ioc_container~register_instance.
    store_object( i_interface = i_interface
                  i_filters   = i_filters
                  i_object    = i_instance ).
  ENDMETHOD.

  METHOD /c2a/if_ioc_container~resolve.
    DATA(interface) = COND #( WHEN i_interface IS NOT INITIAL
                              THEN i_interface
                              ELSE get_return_type( i_increase_depth_by = 1 ) ).

    IF parent IS BOUND.
      TRY.
          r_instance = parent->resolve( i_interface = i_interface
                                        i_filters   = i_filters ).
    ...
    TRY.
        r_instance = instances[ interface = interface
                                filters   = i_filters ]-object.

      CATCH cx_sy_itab_line_not_found.
        r_instance = create_object( i_interface = interface
                                    i_filters   = i_filters ).
        store_object( i_interface = interface
                      i_filters   = i_filters
                      i_object    = r_instance ).
    ENDTRY.
  ENDMETHOD.
