CLASS /c2a/cl_navigation DEFINITION
  PUBLIC
  CREATE PUBLIC.

  PUBLIC SECTION.
    INTERFACES /c2a/if_navigation.

    METHODS constructor
      IMPORTING i_persistence TYPE REF TO /c2a/if_persistence
                i_ddic_access TYPE REF TO /c2a/if_ddic_access
                i_launcher    TYPE REF TO /c2a/if_report_launcher.
  ...
  PRIVATE SECTION.
    CONSTANTS:
      BEGIN OF mass_processing,
        none  TYPE /c2a/nav_mass_processing VALUE space,
        loop  TYPE /c2a/nav_mass_processing VALUE 'L',
        table TYPE /c2a/nav_mass_processing VALUE 'T',
      END OF mass_processing.
    CONSTANTS exit_interface TYPE seoitfname VALUE '/C2A/IF_NAVIGATION' ##NO_TEXT.

    DATA persistence      TYPE REF TO /c2a/if_persistence.
    DATA ddic_access      TYPE REF TO /c2a/if_ddic_access.
    DATA launcher         TYPE REF TO /c2a/if_report_launcher.
    DATA ioc_container    TYPE REF TO /c2a/if_ioc_container.
    DATA navigation_table TYPE /c2a/navigation_tab.
    DATA navigation_exits TYPE /c2a/navigation_exit_tab.

    METHODS ask_for_missing_parameters
      IMPORTING i_target_view      TYPE ddstrucobjname
      CHANGING  c_parameter_values TYPE /c2a/cds_parameter_tab
      RAISING   /c2a/cx_error_message.

    METHODS call_bor_method
      IMPORTING i_navigation   TYPE /c2a/navigation
    ...
    METHODS get_object_from_ioc_container
      IMPORTING i_navigation_exit TYPE /c2a/nav_exit
      RETURNING VALUE(r_object)   TYPE REF TO /c2a/if_navigation
      RAISING   /c2a/cx_error_message.
...
CLASS /c2a/cl_navigation IMPLEMENTATION.
  METHOD ask_for_missing_parameters.
    ...
    LOOP AT dd10bv_tab INTO DATA(dd10bv).
      ...
      ASSIGN c_parameter_values[ cds_view = dd10bv-strucobjn
                                 parname  = dd10bv-parametername ] TO FIELD-SYMBOL(<parameter_value>).
      IF sy-subrc <> 0.
  ...
  METHOD call_bor_method.
    ...
    CALL FUNCTION 'SWO_CREATE'
      EXPORTING
        objtype = i_navigation-object_type
        objkey  = objkey
      IMPORTING
        object  = object
        return  = return.
  ...
  METHOD call_function_module.
    ...
    CALL FUNCTION 'FUNCTION_IMPORT_INTERFACE'
      EXPORTING
        funcname           = i_navigation-function
      TABLES
    ...
    IF input IS NOT BOUND.
      RAISE EXCEPTION TYPE /c2a/cx_error_message
            MESSAGE e005(/c2a/cds_alv) WITH i_navigation-default_parameter i_navigation-function.
    ENDIF.
  ...
  METHOD call_oo_method.
    ...
    IF input IS NOT BOUND.
      RAISE EXCEPTION TYPE /c2a/cx_error_message
            MESSAGE e033(/c2a/cds_alv) WITH i_navigation-method_parameter i_navigation-class i_navigation-method.
    ENDIF.
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE /c2a/cx_error_message
            MESSAGE e035(/c2a/cds_alv) WITH i_navigation-class i_navigation-method.
    ENDIF.
    ...
    TRY.
      ...
      CATCH cx_sy_create_object_error.
        RAISE EXCEPTION TYPE /c2a/cx_error_message
              MESSAGE e036(/c2a/cds_alv) WITH i_navigation-class.

      CATCH cx_sy_dyn_call_error.
        RAISE EXCEPTION TYPE /c2a/cx_error_message
              MESSAGE e034(/c2a/cds_alv) WITH i_navigation-class i_navigation-method.
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD fill_ioc_container.
    ioc_container = NEW /c2a/cl_ioc_container( ).
    LOOP AT navigation_exits INTO DATA(navigation_exit).
      DATA(filters) = VALUE /c2a/if_ioc_container=>ty_filters(
  ...
  METHOD get_object_from_ioc_container.
    ...
    r_object ?= ioc_container->resolve( i_interface = exit_interface
                                        i_filters   = filters ).
  ENDMETHOD.
  ...
  METHOD /c2a/if_navigation~navigate_to_object_mass.
    ...
    TRY.
        ...
        IF navigation-mass_processing = mass_processing-none AND lines( i_selected_rows ) > 1.
          RAISE EXCEPTION TYPE /c2a/cx_error_message MESSAGE e003(/c2a/cds_alv).
        ENDIF.
          ...
          WHEN mass_processing-table.
            ...
            ELSEIF     navigation-class            IS NOT INITIAL
              ...
              call_oo_method( i_navigation    = navigation
                              ...
                              i_selected_rows = i_selected_rows ).
            ELSE.
              RAISE EXCEPTION TYPE /c2a/cx_error_message MESSAGE e004(/c2a/cds_alv).
            ENDIF.
        ENDCASE.

        e_refresh_after = navigation-refresh_after_action.

      CATCH cx_sy_itab_line_not_found.
        RAISE EXCEPTION TYPE /c2a/cx_error_message
              MESSAGE e002(/c2a/cds_alv) WITH i_object i_action.
    ENDTRY.
  ENDMETHOD.

  METHOD /c2a/if_navigation~navigate_to_object_single.
    e_refresh_after = abap_false.
  ...
  METHOD /c2a/if_navigation~navigate_via_association.
