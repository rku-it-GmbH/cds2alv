CLASS /c2a/cl_report_launcher DEFINITION
  PUBLIC
  CREATE PUBLIC.

  PUBLIC SECTION.
    INTERFACES /c2a/if_report_launcher.
    ...
    DATA auth_checker TYPE REF TO /c2a/if_authority_check.
    ...
    METHODS constructor
      IMPORTING i_ddic_access  TYPE REF TO /c2a/if_ddic_access
                ...
                i_generator    TYPE REF TO /c2a/if_report_generator
                i_auth_checker TYPE REF TO /c2a/if_authority_check.
...
CLASS /c2a/cl_report_launcher IMPLEMENTATION.
  METHOD constructor.
  ...
  METHOD ensure_generation.
    ...
    TRY.
        ...
        LOOP AT persistence->get_report_extensions( i_cds_view    = i_cds_view
                                                    i_only_active = abap_false ) INTO DATA(extension).
          extensions_active_since = nmax( val1 = extensions_active_since
                                          val2 = extension-activated_on ).
        ENDLOOP.
  ...
  METHOD /c2a/if_report_launcher~run_in_dark_mode.
  ...
  METHOD /c2a/if_report_launcher~start_report_for_association.
    ...
    LOOP AT program-parameters INTO DATA(parameter).
      ...
      IF sy-subrc = 0.
        INSERT VALUE #( kind    = 'P'
                        selname = parameter-sel_name
                        low     = parameter_value-value )
               INTO TABLE selection_table.
      ENDIF.
    ENDLOOP.

    DATA(memory_id) = CONV /c2a/memory_id( |{ i_source_view }\\{ i_association_name }| ).
  ...
  METHOD /c2a/if_report_launcher~start_report_for_view.
    ensure_generation( i_cds_view ).
    DATA(program) = persistence->get_report_for_cds_view( i_cds_view ).
