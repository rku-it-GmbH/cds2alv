CLASS /c2a/cl_report_strategy DEFINITION
  PUBLIC
  INHERITING FROM /c2a/cl_base_object
  CREATE PUBLIC.

  PUBLIC SECTION.
    INTERFACES /c2a/if_report_strategy.
  ...
  PRIVATE SECTION.
    ...
    METHODS get_extension_parameters
      RETURNING VALUE(r_extension_parameters) TYPE /c2a/extension_parameter_tab.
...
CLASS /c2a/cl_report_strategy IMPLEMENTATION.
  METHOD constructor.
  ...
  METHOD get_program_name.
    TRY.
      ...
      CATCH /c2a/cx_error_message.
        " Noch kein Programm vorhanden
        IF db_view IS NOT INITIAL.
          r_progname = replace( val  = |/C2A/V_{ db_view }|
                                sub  = '/'
                                with = '_'
                                occ  = 0 ).
        ELSE.
          DATA(number) = persistence->get_next_program_number( cds_view ).
          r_progname = |/C2A/N_{ sy-mandt }{ number }|.
        ENDIF.
    ENDTRY.
  ENDMETHOD.

  METHOD /c2a/if_report_strategy~write_source.
    ...
    " TOP Section
    APPEND LINES OF VALUE /c2a/source_line_tab( ( |REPORT { progname }.| )
                                                ( |TABLES: sscrfields.| )
                                                ( |DATA: { template_name } TYPE { cds_view }.|  ) )
           TO r_program-source_lines.
    append_initial_line.

    APPEND LINES OF VALUE /c2a/source_line_tab( ( |CONSTANTS cds_view TYPE /c2a/cds_view_name VALUE '{ cds_view }'.| )
                                                ( |CONSTANTS title TYPE sytitle VALUE '{ description }'.| ) ) TO r_program-source_lines.
    append_initial_line.

    APPEND LINES OF VALUE /c2a/source_line_tab( ( |DATA factory TYPE REF TO /c2a/if_object_factory.      | )
                                                ( |DATA controller TYPE REF TO /c2a/if_report_controller.| )
                                                ( |DATA message TYPE REF TO /c2a/cx_error_message.       | ) ) TO r_program-source_lines.
    append_initial_line.

    APPEND LINES OF VALUE /c2a/source_line_tab(
    ...
    APPEND LINES OF VALUE /c2a/source_line_tab(
        ...
        ( |PARAMETERS p_no_max TYPE /c2a/no_max USER-COMMAND no_max.    | )
        ( |SELECTION-SCREEN SKIP.                                       | )
        ( |PARAMETERS p_selext TYPE /c2a/report_extension_name AS LISTBOX visible length 40 USER-COMMAND switch_sel MODIF ID sel.| )
        ( |PARAMETERS p_disext TYPE /c2a/report_extension_name AS LISTBOX visible length 40 USER-COMMAND switch_dis MODIF ID dis.| )
        ( |SELECTION-SCREEN SKIP.                                       | )
        ( |PARAMETERS p_vari   TYPE slis_vari.                          | )
        ( |PARAMETERS p_split  TYPE xfeld DEFAULT abap_false.           | )
        ( |PARAMETERS p_forall TYPE xfeld NO-DISPLAY.                   | )
        ( |PARAMETERS p_mem_id TYPE /c2a/memory_id NO-DISPLAY.          | )
    ...
    " End of Subscreen
    APPEND LINES OF VALUE /c2a/source_line_tab( ( |SELECTION-SCREEN END OF BLOCK sub.                  | )
                                                ( |SELECTION-SCREEN END OF SCREEN { r_program-dynpro }.| ) ) TO r_program-source_lines.
    append_initial_line.

    " Include in main selection screen
    APPEND LINES OF VALUE /c2a/source_line_tab( ( |SELECTION-SCREEN BEGIN OF BLOCK main WITH FRAME TITLE text-sub.| )
                                                ( |SELECTION-SCREEN INCLUDE BLOCKS sub.                           | )
                                                ( |SELECTION-SCREEN END OF BLOCK main.                            | ) ) TO r_program-source_lines.
    append_initial_line.

    " LOAD-OF-PROGRAM
    APPEND LINES OF VALUE /c2a/source_line_tab( ( |LOAD-OF-PROGRAM.                                               | )
                                                ( |  TRY.                                                         | )
    ...
    " INITIALIZATION
    APPEND LINES OF VALUE /c2a/source_line_tab( ( |INITIALIZATION.                                 | )
                                                ( |  sy-title = title.                             | )
                                                ( |                                                | )
                                                ( |  GET PARAMETER ID '/C2A/NO_MAX' FIELD p_no_max.| )
                                                ( |                                                | )
                                                ( |  TRY.                                          | )
    ...
    APPEND LINES OF VALUE /c2a/source_line_tab( ( |AT SELECTION-SCREEN OUTPUT.                      | )
                                                ( |  TRY.                                           | )
    ...
    " AT-SELECTION-SCREEN
    APPEND LINES OF VALUE /c2a/source_line_tab( ( |AT SELECTION-SCREEN.                                       | )
                                                ( |  TRY.                                                     | )
    ...
    APPEND LINES OF VALUE /c2a/source_line_tab( ( |AT SELECTION-SCREEN ON VALUE-REQUEST FOR p_vari.     | )
    ...
    " START-OF-SELECTION
    APPEND LINES OF VALUE /c2a/source_line_tab( ( |START-OF-SELECTION.                              | )
                                                ( |  TRY.                                           | )
  ...
  METHOD /c2a/if_report_strategy~write_textpool.
    LOOP AT field_properties_tab INTO DATA(field_properties).
      APPEND VALUE #( id    = 'S'
    ...
    APPEND VALUE #( id    = 'I'
                    key   = 'SUB'
                    entry = TEXT-i01 ) TO r_textpool.
    APPEND VALUE #( id    = 'S'
                    key   = 'P_MAXREC'
                    entry = |        { TEXT-s01 }| ) TO r_textpool.
    APPEND VALUE #( id    = 'S'
                    key   = 'P_NO_MAX'
                    entry = |        { TEXT-s02 }| ) TO r_textpool.
    APPEND VALUE #( id    = 'S'
                    key   = 'P_FORALL'
                    entry = |        { TEXT-s03 }| ) TO r_textpool.
    APPEND VALUE #( id    = 'S'
                    key   = 'P_MEM_ID'
                    entry = |        { TEXT-s04 }| ) TO r_textpool.
    APPEND VALUE #( id    = 'S'
                    key   = 'P_SPLIT'
                    entry = |        { TEXT-s05 }| ) TO r_textpool.
    APPEND VALUE #( id    = 'S'
                    key   = 'P_SELEXT'
                    entry = |        { TEXT-s06 }| ) TO r_textpool.
    APPEND VALUE #( id    = 'S'
                    key   = 'P_DISEXT'
                    entry = |        { TEXT-s07 }| ) TO r_textpool.
    APPEND VALUE #( id    = 'S'
                    key   = 'P_VARI'
                    entry = |        { TEXT-s08 }| ) TO r_textpool.

    LOOP AT get_extension_parameters( ) INTO DATA(parameter).
      INSERT VALUE #( id    = 'S'
                      key   = parameter-parameter_name
                      entry = |        { parameter-parameter_text }| )
