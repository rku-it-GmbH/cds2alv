CLASS /c2a/cl_sadl_mapping_provider DEFINITION
  PUBLIC
  INHERITING FROM cl_sadl_mp_entity FINAL
  CREATE PUBLIC.
...
CLASS /c2a/cl_sadl_mapping_provider IMPLEMENTATION.
...
*        ENDIF.
        ...
        INSERT VALUE #( name    = id
                        type    = entity_type
                        binding = entity_id ) INTO TABLE ms_sadl_definition-data_sources.

        INSERT VALUE #( name          = id
                        anchor        = abap_true
                        data_source   = id
                        max_edit_mode = 'RO'
                        id            = id ) INTO TABLE ms_sadl_definition-structures.
...
*        INSERT VALUE #( name = id  anchor = abap_true  data_source = id  max_edit_mode = 'RO'  id = id exposure = exposure ) INTO TABLE ms_sadl_definition-structures.
        ...
        INSERT VALUE #( name         = 'SADL_QUERY'
                        structure_id = id
                        id           = 'QUERY' ) INTO TABLE ms_sadl_definition-mapped_queries ASSIGNING FIELD-SYMBOL(<query>).

        DATA(entity_consumption_info) = cl_sadl_entity_factory=>get_instance( )->get_entity_consumption_info(
                                            iv_id   = entity_id
                                            iv_type = entity_type ).
        ...
        LOOP AT elements ASSIGNING FIELD-SYMBOL(<element>).
          DATA(tabix) = sy-tabix.
          INSERT VALUE #( name         = <element>-name
                          binding      = <element>-name
                          structure_id = id
                          id           = |X{ tabix }| ) INTO TABLE ms_sadl_definition-attributes.
          IF <element>-text_search-allowed = abap_true.
            ASSIGN <query>-input_parameters[ type = 'SEARCH_TEXT' ] TO FIELD-SYMBOL(<search>).
            IF sy-subrc <> 0.
              APPEND VALUE #( id   = 'SEARCH'
                              name = 'SEARCH_TEXT'
                              type = 'SEARCH_TEXT' ) TO <query>-input_parameters ASSIGNING <search>.
            ENDIF.
            APPEND VALUE #( ip_id   = 'SEARCH'
                            binding = <element>-name ) TO <search>-attributes.
          ENDIF.
        ENDLOOP.

      CATCH cx_sadl_static INTO DATA(previous).
        RAISE EXCEPTION TYPE cx_bsa_compile_time
          EXPORTING textid   = cx_bsa_compile_time=>cx_bsa_sadl_xml
                    previous = previous.
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD constructor.
    ...
    DATA(uuid) = CONV if_sadl_types=>ty_uuid( |/C2A/CDS_ALV:{ i_entity->get_id( ) }| ).
