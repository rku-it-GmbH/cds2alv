CLASS lcl_sadl_cond_provider_forall DEFINITION FINAL
  CREATE PUBLIC.
  ...
  PRIVATE SECTION.
    ...
    DATA source_view       TYPE /c2a/cds_view_name.
    DATA association_name  TYPE ddassociationname.
    DATA source_parameters TYPE /c2a/cds_parameter_tab.
...
CLASS lcl_sadl_cond_provider_forall IMPLEMENTATION.
  ...
  METHOD if_sadl_condition_provider~get_condition.
    ...
    FIELD-SYMBOLS <prev_dd05bv>  TYPE dd05bv.
    ...
    TRY.
        ddic_access->get_conditions_for_association( EXPORTING i_source_view      = source_view
                                                               i_association_name = association_name
                                                     IMPORTING e_target_view      = DATA(target_view)
                                                               e_dd05bv_tab       = DATA(dd05bv_tab) ).

      CATCH /c2a/cx_error_message.
        RAISE EXCEPTION TYPE cx_sadl_contract_violation
          EXPORTING textid = cx_sadl_contract_violation=>entity_not_found
      ...
      LOOP AT dd05bv_tab ASSIGNING <curr_dd05bv>.
        ...
        CASE <curr_dd05bv>-side_info.
          ...
          WHEN 'L1'. " Literal
            ...
            range-low    = replace( val  = range-low
                                    sub  = `'`
                                    with = ``
                                    occ  = 1 ).
            range-low    = replace( val  = range-low
          ...
          WHEN 'P1'. " Parameter
            DATA(parname) = substring_after( val = <curr_dd05bv>-fieldname
                                             sub = '$' ).
            READ TABLE source_parameters INTO DATA(source_parameter)
                 WITH TABLE KEY cds_view = source_view
                                parname  = parname.
            IF sy-subrc = 0.
              range-sign   = 'I'.
              range-option = <curr_dd05bv>-operator.
          ...
          WHEN 'S1'. " Session variable
            DATA(session_variable) = substring_after( val = <curr_dd05bv>-fieldname
                                                      sub = `$SESSION.` ).
            range-sign   = 'I'.
            range-option = <curr_dd05bv>-operator.
        ...
        cl_sadl_condition_generator=>append_sadl_condition(
          EXPORTING iv_attribute       = CONV #( <curr_dd05bv>-fieldname_t )
                    iv_option          = range-option
                    iv_low             = range-low
          CHANGING  ct_sadl_conditions = et_sadl_condition ).
        ...
        ASSIGN dd05bv_tab[ inner_tabix ] TO <prev_dd05bv>.
      ENDLOOP.
