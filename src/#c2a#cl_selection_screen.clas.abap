CLASS /c2a/cl_selection_screen DEFINITION
  PUBLIC
  INHERITING FROM /c2a/cl_base_object
  CREATE PUBLIC.

  PUBLIC SECTION.
    INTERFACES /c2a/if_condition_provider.
    INTERFACES /c2a/if_selection_screen.

    ALIASES get_dynpro_field      FOR /c2a/if_selection_screen~get_dynpro_field.
    ALIASES get_parameters        FOR /c2a/if_selection_screen~get_parameters.
    ALIASES get_selections        FOR /c2a/if_selection_screen~get_selections.
    ALIASES read_selection_screen FOR /c2a/if_selection_screen~read_selection_screen.
  ...
  PROTECTED SECTION.
    ...
    DATA selection_mappings TYPE /c2a/selopts_mapping_tab.
    DATA parameter_mappings TYPE /c2a/parameter_mapping_tab.
...
CLASS /c2a/cl_selection_screen IMPLEMENTATION.
  ...
  METHOD evaluate_annotations.
    ...
    LOOP AT element_annotations ASSIGNING FIELD-SYMBOL(<element_annotation_group>)
      ...
      TRY.
          DATA(sel_name) = selection_mappings[ cds_view  = cds_view
                                               fieldname = fieldname ]-sel_name. "#EC CI_HASHSEQ

          IF properties-selection_type IS NOT INITIAL.
            INSERT VALUE #( name = sel_name
                            kind = kind-select_option )
  ...
  METHOD /c2a/if_condition_provider~get_parameters.
  ...
  METHOD /c2a/if_condition_provider~get_selections.
    CLEAR: e_where_tab,
           e_field_ranges,
           e_maxrec,
           e_no_max.
    ...
    LOOP AT selection_table INTO DATA(selection) WHERE kind = kind-select_option.
      READ TABLE selection_mappings INTO DATA(selection_mapping)
           WITH TABLE KEY progname = progname
                          sel_name = selection-selname.
      IF sy-subrc <> 0.
        CONTINUE.
      ENDIF.

      ASSIGN e_field_ranges[ fieldname = selection_mapping-fieldname ] TO FIELD-SYMBOL(<field_ranges>).
      IF sy-subrc <> 0.
    ...
    TRY.
        e_no_max = selection_table[ kind    = kind-parameter
                                    selname = fixed_parameters-no_max ]-low.
      CATCH cx_sy_itab_line_not_found.
        e_no_max = abap_false.
    ENDTRY.
    ...
    ELSE.
      TRY.
          e_maxrec = selection_table[ kind    = kind-parameter
                                      selname = fixed_parameters-max_records ]-low.
        CATCH cx_sy_itab_line_not_found.
  ...
  METHOD /c2a/if_selection_screen~apply_restriction.
  ...
  METHOD /c2a/if_selection_screen~apply_selection_texts.
    CALL FUNCTION 'SELECTION_TEXTS_MODIFY'
      EXPORTING
        program                     = progname
      TABLES
        seltexts                    = selection_texts
      EXCEPTIONS
        program_not_found           = 1
  ...
  METHOD /c2a/if_selection_screen~get_dynpro_field.
    CLEAR: e_parameter,
           e_select_options.
  ...
  METHOD /c2a/if_selection_screen~modify_screen.
    read_selection_screen( ).

    DATA(selection_values) = VALUE vrm_values( FOR x_selection IN persistence->get_report_extensions(
                                                                      i_cds_view       = cds_view
                                                                      i_only_selection = abap_true )
                                               ( key = x_selection-extension_name text = x_selection-selection_text ) ).

    IF selection_values IS NOT INITIAL.
      INSERT VALUE #( key  = space
                      text = TEXT-sel ) INTO selection_values INDEX 1.
    ENDIF.

    CALL FUNCTION 'VRM_SET_VALUES'
      EXPORTING
        id     = CONV vrm_id( fixed_parameters-selection )
    ...
    DATA(display_values) = VALUE vrm_values( FOR x_display IN persistence->get_report_extensions(
                                                                  i_cds_view     = cds_view
                                                                  i_only_display = abap_true )
                                             ( key = x_display-extension_name text = x_display-display_text ) ).

    IF display_values IS NOT INITIAL.
      INSERT VALUE #( key  = space
                      text = TEXT-dis ) INTO display_values INDEX 1.
    ENDIF.

    CALL FUNCTION 'VRM_SET_VALUES'
      EXPORTING
        id     = CONV vrm_id( fixed_parameters-display )
  ...
  METHOD /c2a/if_selection_screen~read_selection_screen.
    DATA(rsparams_tab) = CORRESPONDING rsparams_tt( selection_table ).
  ...
  METHOD /c2a/if_selection_screen~set_dynpro_field.
    ...
    ELSEIF i_select_options IS NOT INITIAL.
      ...
      LOOP AT i_select_options INTO DATA(select_option).
        INSERT CORRESPONDING #( BASE ( VALUE #( selname = i_sel_name
                                                kind    = 'S' ) ) select_option )
  ...
  METHOD /c2a/if_selection_screen~value_help_for_field.
    IF value_help IS NOT BOUND.
      RETURN.
    ENDIF.

    TRY.
        DATA(fieldname) = selection_mappings[ progname = progname
                                              sel_name = i_sel_name ]-fieldname.
    ...
    TRY.
        DATA(parameter_name) = parameter_mappings[ progname = progname
                                                   sel_name = i_sel_name ]-parname.
