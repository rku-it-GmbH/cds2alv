CLASS /c2a/cl_table_container DEFINITION
  PUBLIC
  INHERITING FROM /c2a/cl_base_object
  CREATE PUBLIC.

  PUBLIC SECTION.
    INTERFACES /c2a/if_table_container.
  ...
  PROTECTED SECTION.
    ...
    DATA special_columns           TYPE /c2a/special_columns.
...
CLASS /c2a/cl_table_container IMPLEMENTATION.
  METHOD adjust_table.
    ...
    LOOP AT <table> ASSIGNING FIELD-SYMBOL(<line>).
      ...
      IF sy-subrc = 0.
        LOOP AT criticality_mapping_table INTO DATA(mapping).
          ...
          ASSIGN <color_table>[ fname = mapping-data_field ] TO FIELD-SYMBOL(<color>).
          IF sy-subrc <> 0.
  ...
  METHOD evaluate_annotations.
    ...
    special_columns = VALUE /c2a/special_columns( ).
    criticality_mapping_table = VALUE ty_criticality_mapping_table( ).

    special_columns-index_fieldname = get_unique_fieldname( i_components = components
                                                            i_fieldname  = default_fieldname-index ).
    INSERT VALUE #( name = special_columns-index_fieldname
                    type = CAST #( cl_abap_elemdescr=>get_i( ) ) ) INTO TABLE components.

    special_columns-count_fieldname = get_unique_fieldname( i_components = components
                                                            i_fieldname  = default_fieldname-count ).
    INSERT VALUE #( name = special_columns-count_fieldname
                    type = CAST #( cl_abap_elemdescr=>get_i( ) ) ) INTO TABLE components.

    special_columns-box_fieldname = get_unique_fieldname( i_components = components
                                                          i_fieldname  = default_fieldname-box ).
    INSERT VALUE #( name = special_columns-box_fieldname
                    type = CAST #( cl_abap_typedescr=>describe_by_name( 'XFELD' ) ) ) INTO TABLE components.

    special_columns-system_fieldname = get_unique_fieldname( i_components = components
                                                             i_fieldname  = default_fieldname-system ).
    INSERT VALUE #( name = special_columns-system_fieldname
                    type = CAST #( cl_abap_typedescr=>describe_by_name( 'SYST_SYSID' ) ) ) INTO TABLE components.

    special_columns-client_fieldname = get_unique_fieldname( i_components = components
                                                             i_fieldname  = default_fieldname-client ).
    INSERT VALUE #( name = special_columns-client_fieldname
                    type = CAST #( cl_abap_typedescr=>describe_by_name( 'MANDT' ) ) ) INTO TABLE components.

    DATA(update_enabled) = xsdbool( line_exists( entity_annotations[ annoname = 'OBJECTMODEL.UPDATEENABLED'
                                                                     value    = 'true' ] ) ).
    " TODO: variable is assigned but never used (ABAP cleaner)
    DATA(delete_enabled) = xsdbool( line_exists( entity_annotations[ annoname = 'OBJECTMODEL.DELETEENABLED'
                                                                     value    = 'true' ] ) ).

    IF update_enabled = abap_true.
      special_columns-style_fieldname = get_unique_fieldname( i_components = components
                                                              i_fieldname  = default_fieldname-style ).
      INSERT VALUE #( name = special_columns-style_fieldname
                      type = CAST #( cl_abap_typedescr=>describe_by_name( 'LVC_T_STYL' ) ) ) INTO TABLE components.
    ENDIF.
    ...
    LOOP AT element_annotations INTO DATA(element_annotation).
      IF element_annotation-annoname CP 'UI.LINEITEM*.CRITICALITY'.
        IF special_columns-color_fieldname IS INITIAL.
          special_columns-color_fieldname = get_unique_fieldname( i_components = components
                                                                  i_fieldname  = default_fieldname-color ).
          INSERT VALUE #( name = special_columns-color_fieldname
                          type = CAST #( cl_abap_typedescr=>describe_by_name( 'LVC_T_SCOL' ) ) ) INTO TABLE components.
        ENDIF.
  ...
  METHOD get_unique_fieldname.
    IF line_exists( i_components[ name = i_fieldname ] ).
      r_fieldname = get_unique_fieldname( i_components = i_components
                                          i_fieldname  = |_{ i_fieldname }| ).
    ELSE.
  ...
  METHOD /c2a/if_table_container~get_line_descriptor.
    r_line_descriptor = line_descriptor.
  ENDMETHOD.

  METHOD /c2a/if_table_container~get_ref_to_table.
    r_ref_to_table = ref_to_table.
  ENDMETHOD.

  METHOD /c2a/if_table_container~get_special_columns.
    r_special_columns = special_columns.
  ENDMETHOD.

  METHOD /c2a/if_table_container~get_table.
    ASSIGN ref_to_table->* TO FIELD-SYMBOL(<table>).
    e_table = <table>.
  ENDMETHOD.

  METHOD /c2a/if_table_container~get_table_descriptor.
    r_table_descriptor = table_descriptor.
  ENDMETHOD.

  METHOD /c2a/if_table_container~is_in_edit_mode.
    r_in_edit_mode = in_edit_mode.
  ENDMETHOD.

  METHOD /c2a/if_table_container~set_ref_to_table.
    IF NOT table_descriptor->applies_to_data_ref( i_ref_to_table ).
      RAISE EXCEPTION TYPE /c2a/cx_error_message
            MESSAGE e018(/c2a/cds_alv) WITH cds_view.
    ENDIF.
  ...
  METHOD /c2a/if_table_container~toggle_change_mode.
