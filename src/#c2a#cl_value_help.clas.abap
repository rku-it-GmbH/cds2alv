CLASS /c2a/cl_value_help DEFINITION
  PUBLIC
  INHERITING FROM /c2a/cl_base_object
  CREATE PUBLIC.

  PUBLIC SECTION.
    INTERFACES /c2a/if_value_help.
  ...
  PROTECTED SECTION.
    DATA value_helps TYPE /c2a/valuehelp_definition_tab.
    ...
    METHODS value_help_by_association
      IMPORTING i_fieldname        TYPE fieldname
                ...
                i_parameters       TYPE /c2a/cds_parameter_tab OPTIONAL
    ...
    METHODS value_help_by_target_entity
      IMPORTING i_target_entity      TYPE /c2a/cds_view_name
                i_target_element     TYPE fieldname
                i_additional_binding TYPE /c2a/valuehelp_binding_tab OPTIONAL
                i_parameters         TYPE /c2a/cds_parameter_tab     OPTIONAL
                i_field_ranges       TYPE rsds_frange_t              OPTIONAL
                i_selected_row       TYPE any                        OPTIONAL
                i_display            TYPE abap_bool
    ...
    METHODS choose_value_help
      CHANGING c_value_helps TYPE /c2a/valuehelp_definition_tab
      RAISING  /c2a/cx_error_message.
...
CLASS /c2a/cl_value_help IMPLEMENTATION.
  METHOD choose_value_help.
    ...
    IF sy-subrc = 0.
      DATA(value_help_index) = CONV sytabix( substring_before( val = selected_option-varoption
                                                               sub = ':' ) ).
    ...
    LOOP AT element_annotations ASSIGNING FIELD-SYMBOL(<element_annotation_group>)
      ...
      LOOP AT GROUP <element_annotation_group> ASSIGNING FIELD-SYMBOL(<element_annotation>).
        IF <element_annotation>-annoname = 'CONSUMPTION.VALUEHELP'.
          DATA(value_help) = VALUE /c2a/valuehelp_definition( cds_view  = cds_view
                                                              fieldname = fieldname ).
          value_help-association_name = remove_quotes( <element_annotation>-value ).
          INSERT value_help INTO TABLE value_helps.
        ENDIF.

        IF <element_annotation>-annoname NP 'CONSUMPTION.VALUEHELPDEFINITION$*$.*'.
          CONTINUE.
        ENDIF.

        SPLIT <element_annotation>-annoname AT '$' INTO DATA(prefix) DATA(value_help_index) DATA(property).

        ASSIGN value_helps[ cds_view         = cds_view
                            fieldname        = fieldname
                            value_help_index = value_help_index ] TO FIELD-SYMBOL(<value_help>).
        IF sy-subrc <> 0.
        ...
        ASSIGN value_helps[ cds_view         = cds_view
                            fieldname        = fieldname
                            value_help_index = value_help_index ] TO <value_help>.
        IF sy-subrc <> 0.
          CONTINUE.
        ENDIF.

        ASSIGN <value_help>-additional_binding[ index = binding_index ] TO FIELD-SYMBOL(<additional_binding>).
        IF sy-subrc <> 0.
  ...
  METHOD value_help_by_association.
    " TODO: parameter I_FIELD_RANGES is never used (ABAP cleaner)
        ...
        LOOP AT i_additional_binding INTO DATA(binding).
          ...
          IF binding-source_element IS NOT INITIAL.
            IF i_selected_row IS NOT INITIAL.
              ...
              IF sy-subrc = 0.
                ...
                source_field_range = VALUE #( fieldname = binding-source_element
                                              selopt_t  = select_options ).
              ENDIF.
          ...
          ELSEIF binding-source_parameter IS NOT INITIAL.
            READ TABLE i_parameters INTO DATA(source_parameter)
                 WITH TABLE KEY cds_view = cds_view
                                parname  = binding-source_parameter.
          ENDIF.
            ...
            IF binding-usage <> '#RESULT'.
              IF target_field_range-selopt_t IS NOT INITIAL.
                sadl_runtime->if_sadl_query_fetch~register_condition_provider(
                    ...
                    )->in_range( name     = CONV #( target_field_range-fieldname )
  ...
  METHOD /c2a/if_value_help~value_help_for_element.
    TRY.
        e_processed = abap_false.

        DATA(value_helps_for_field) = VALUE /c2a/valuehelp_definition_tab( FOR x_value_help IN value_helps
  ...
  METHOD /c2a/if_value_help~value_help_for_parameter.
    TRY.
        e_processed = abap_false.

        DATA(value_helps_for_param) = VALUE /c2a/valuehelp_definition_tab( FOR x_value_help IN value_helps
