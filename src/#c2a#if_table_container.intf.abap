...
INTERFACE /c2a/if_table_container PUBLIC.
  ...
  "! @raising   /c2a/cx_error_message | Occurs when the table type does not match the container's expectations.
  ...
  METHODS get_special_columns
    RETURNING VALUE(r_special_columns) TYPE /c2a/special_columns.

  "! Switches between display and change mode.
  "! @parameter i_alv_grid          | The ALV Grid that display the table
  "! @raising   /c2a/cx_error_message | Occurs when Update is not enabled
