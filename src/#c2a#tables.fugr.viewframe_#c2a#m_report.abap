*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_/C2A/M_REPORT
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_/C2A/M_REPORT       .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_/C2A/M_REPORT'
         EXPORTING
              FCODE          = READ
...
EXTRACT = /C2A/M_REPORT_EXTRACT
TOTAL = /C2A/M_REPORT_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_/C2A/M_REPORT'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_/C2A/M_REPORT-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = /C2A/M_REPORT_EXTRACT
TOTAL = /C2A/M_REPORT_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_/C2A/M_REPORT-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_/C2A/M_REPORT'
...
TOTAL = /C2A/M_REPORT_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_/C2A/M_REPORT-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_/C2A/M_REPORT-UPD_FLAG NE SPACE.
